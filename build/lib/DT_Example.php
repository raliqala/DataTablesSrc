...
class DT_Example
{
    ...
    public function __construct($file = null, $template = null, $path_resolver = null, $libs = array())
    {
        if ($file !== null) {
            $this->file($file);
        }
    ...
    private function _column($name, $type, $row = null)
    {
        ...
        switch ($name) {
            case '':
                if ($type === 'title') {return '';} else if ($type === 'data') {return '';}
                break;

            case 'id':
                if ($type === 'title') {return 'ID';} else if ($type === 'data') {return $row['id'];}
                break;

            case 'name':
                if ($type === 'title') {return 'Name';} else if ($type === 'data') {return $row['first_name'] . ' ' . $row['last_name'];}
                break;

            case 'name-attr':
                if ($type === 'title') {return 'Name';} else if ($type === 'data') {
            ...
            case 'first_name':
                if ($type === 'title') {return 'First name';} else if ($type === 'data') {return $row['first_name'];}
                break;

            case 'last_name':
                if ($type === 'title') {return 'Last name';} else if ($type === 'data') {return $row['last_name'];}
                break;

            case 'age':
                if ($type === 'title') {return 'Age';} else if ($type === 'data') {return $row['age'];}
                break;

            case 'position':
                if ($type === 'title') {return 'Position';} else if ($type === 'data') {return $row['position'];}
                break;

            case 'salary':
                if ($type === 'title') {return 'Salary';} else if ($type === 'data') {return '$' . number_format($row['salary']);}
                break;

            case 'salary-attr':
                if ($type === 'title') {return 'Salary';} else if ($type === 'data') {return '<td data-order="' . $row['salary'] . '">$' . number_format($row['salary']) . '/y</td>';}
                break;

            case 'salary-plain':
                if ($type === 'title') {return 'Salary';} else if ($type === 'data') {return $row['salary'];}
                break;

            case 'start_date':
                if ($type === 'title') {return 'Start date';} else if ($type === 'data') {return str_replace('/', '-', $row['start_date']);}
                break;

            case 'start_date-attr':
                if ($type === 'title') {return 'Start date';} else if ($type === 'data') {
            ...
            case 'extn':
                if ($type === 'title') {return 'Extn.';} else if ($type === 'data') {return $row['extn'];}
                break;

            case 'email':
                if ($type === 'title') {return 'E-mail';} else if ($type === 'data') {return $row['email'];}
                break;

            case 'progress':
                if ($type === 'title') {return 'Progress';} else if ($type === 'data') {return $row['extn'];}
                break;

            case 'office':
                if ($type === 'title') {return 'Office';} else if ($type === 'data') {return $row['office'];}
                break;

            case 'sequence':
                if ($type === 'title') {return 'Seq.';} else if ($type === 'data') {return $row['sequence'];}
                break;

            case 'symbol':
                if ($type === 'title') {return 'Symbol';} else if ($type === 'data') {return $row['symbol'];}
                break;

            case 'price':
                if ($type === 'title') {return 'Price';} else if ($type === 'data') {return $row['price'];}
                break;

            case 'difference':
                if ($type === 'title') {return 'Difference';} else if ($type === 'data') {return '';}
                break;

            case 'history':
                if ($type === 'title') {return 'Last';} else if ($type === 'data') {return $row['last'];}
                break;
    ...
    public function build_table($type, $idIn = null)
    {
        ...
        } else if (strpos($framework, 'flowbite') !== false) {
            $class = str_replace('display', 'table is-striped', $class);
        }
